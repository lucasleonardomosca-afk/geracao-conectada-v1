...
const Navbar: React.FC<NavbarProps> = ({ onNavClick, onToggleFont, onToggleContrast, highContrast }) => {
  ...
  const textColor = highContrast ? 'text-yellow-300' : 'text-[#0F172A]';
  
  const btnClassBase = `h-[64px] px-8 rounded-2xl font-black text-lg transition-all active:scale-95 flex items-center justify-center gap-3 border-2`;
  
  const btnStyle = highContrast 
    ? `${btnClassBase} border-yellow-300 bg-black text-yellow-300` 
    : `${btnClassBase} border-slate-200 bg-white text-slate-700 hover:bg-slate-50 hover:border-[#059669] hover:text-[#059669] shadow-sm`;
      ...
      <div className="max-w-7xl mx-auto px-8 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-3xl md:text-4xl font-[900] tracking-tighter uppercase leading-none font-inter text-[#0F172A]">{BRAND_NAME}</span>
            <span className="text-[11px] font-[800] tracking-[0.4em] text-[#059669] mt-1 uppercase">ENSINO COM PACIÊNCIA</span>
          </div>
        </a>
        
        <div className="flex flex-wrap justify-center items-center gap-4">
          <button onClick={onToggleFont} className={btnStyle}>
            <span className="text-xl">🔍</span> Texto maior
          </button>
          <button onClick={onToggleContrast} className={btnStyle}>
            <span className="text-xl">🎨</span> Mudar cores
          </button>
        </div>
      </div>
    </nav>
